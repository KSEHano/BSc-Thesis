...
Bellow the BLEU scores, results of the study, and correlation are presented the results are all tested for significance with a pair-wise t-test. The test was two-tied and $\alpha$ was set to 0.1. Significant values are marked as bold. For all cases bold means that the results are significantly different from all other results.
