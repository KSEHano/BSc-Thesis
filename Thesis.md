...
where N is equal to 4, the biggest n-grams considered and w_i are the weights for the n-grams and $w_i = \frac{1}{N}$ [@BLEU]. On a corpus or several sentences the modified precision is then the sum of the the clipped n-gram matches of all sentences divided by the number of candidate n-grams. This is done to avoid that a translation is entirely made out of high frequency the n-gram counts are modified, i.e. the maximum number of occurrences is counted in the reference sentences and all matches are added together to give the modifies n-gram count.
