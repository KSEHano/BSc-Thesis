...
: **Example Segments German** {#tbl:Dataexample_DE}
...
: **Example Segments Czech** {#tbl:Dataexample_CZ}
...
Bellow the BLEU scores, results of the study, and correlation are presented the results are all tested for significance with a pair-wise t-test. To be significant the p-value had to be below 0.05 as ALPHA was 0.1 (two-tied). Significant values are marked as bold. For all cases bold means that the results are significantly different from all other results.
...
In the results for one against all with the human as a hypothesis, the score for the human translation is significantly lower than the other translations. It seems to be not as good of a translation as the others. In the way, BLEU works it compares a hypothesis translation to at minimum one reference translation. For that reason, any result only shows how similar a translation is to the reference translation(s). Each translation has all other translations as references so the human translation has only the MT as references and the result shows that the human translation is not as close to MT as the other translations. This also explains the increase in the BLEU score as soon as the MTs are included as references. They are closer to each other than to the human translation and therefore generate better scores for each other. They are not equal but even two different human translations would score below 1 [@BLEU]. This shows one of the shortcomings of BLEU, it is only as good as the reference translation(s) given.
